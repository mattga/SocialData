...
import (
	...
	"github.com/mattga/goauth2/oauth"
...
func main() {
	...
	for _, searchTerm := range Conf.YTSearchTerms {
		fmt.Printf("New query for comments on %s\n", searchTerm)

		fname := strings.Replace(searchTerm, " ", "-", -1)
		f, err := os.OpenFile(fmt.Sprintf("data/yt_%s.txt", fname), os.O_TRUNC|os.O_CREATE|os.O_WRONLY, 0600)
		if err != nil {
			panic(err)
		}

		for cname, cid := range Conf.YTChannels {
			call := service.CommentThreads.List("snippet")
			call.MaxResults(100)
			call.TextFormat("plainText")
			call.SearchTerms(searchTerm)
			resp, err := call.AllThreadsRelatedToChannelId(cid).Do()
			if err != nil {
				panic(err)
			}
			...
			for {
				...
				} else if resp.PageInfo == nil {
					fmt.Println("NIL RESP")
				}

				count = count + resp.PageInfo.TotalResults
				fmt.Printf("\tFetched comments from '%s' (%d)\n", cname, count)

				for _, thread := range resp.Items {
					comment := thread.Snippet.TopLevelComment
					text := strings.Replace(comment.Snippet.TextDisplay, "\n", " ", -1)

					if len(text) < 500 {
						data := fmt.Sprintf("%s, %s, %s\n", comment.Id, comment.Snippet.PublishedAt, text)
				...
				if resp, err = call.PageToken(resp.NextPageToken).Do(); err != nil {
					panic(err)
				}
			}
		}
